...
This process will assume ANTLR 4.9.3 is already installed on your partition of the web server (if not, see [installation process](https://github.com/antlr/antlr4/blob/master/doc/getting-started.md) for ANTLR on GitHub).
 ...
 1. Due to the nature of packages, create a directory of the form `my_dir/org/meldr/compiler/` somewhere on the server. ("my_dir" is arbitrary)
 2. Place all the java source files listed in previous section as well as MeldrLang.g4 in the compiler folder created in Step 1. 
 3. Go back to `my_dir` and place Micro.sh there.
 4. Change the CLASSPATH environmental variable in your bash profile to point to `my_dir`. 
> Example: `/home/18/n01551618/compiler/Step4:$CLASSPATH`, where Step4 in this context is `my_dir`.
